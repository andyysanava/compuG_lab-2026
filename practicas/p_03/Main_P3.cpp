...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// view = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 3,-700.0f ) ); // use with orthographic projection
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(0.0f, -10.0f, -20.0f)); // use to compare orthographic and perspective projection
		...
		for (int i = 0; i < total; i++) {
			...
			// model = glm::translate(model, translates[i % total]);
			model = glm::translate(model, translates[0]);
