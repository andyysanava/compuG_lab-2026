...
int main() {
	...
	float vertices[] = {
		-0.5f, -0.5f, 0.5f, 1.0f, 0.0f,0.0f,//Front | x, y, z, r, g, b 
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 beemodel=glm::mat4(1);
		glm::mat4 view=glm::mat4(1);
	

		view = glm::translate(view, glm::vec3(movX,movY, movZ));
		view = glm::rotate(view, glm::radians(rot), glm::vec3(0.0f, 1.0f, 0.0f));
		beemodel = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -2.0f, -30.0f));
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
                ...
                // Ejecuta una línea así para cambiar el color (se va a aplicar
                // a todo lo que dibujes a partir de ahora hasta que lo vuelvas
                // a llamar con otros valores )
		glUniform3f(colorLoc, 1.0f, 0.0f, 1.0f);

		glBindVertexArray(VAO);
	
		//Hacemos la cara de la abeja
		//Amarillo 
		...
		glUniform3f(colorLoc, 0.0f, 0.0f, 0.0f);
		model = beemodel;
		model = glm::translate(model, glm::vec3(5.0f, -2.0f, 7.0f));
		model = glm::translate(model, glm::vec3(-0.5f, -0.5f, -0.5f));
		model = glm::scale(model, glm::vec3(1.0f, 2.0f, 1.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, glm::vec3(-0.5f, -0.5f, -0.5f));
		model = glm::scale(model, glm::vec3(6.0f, 1.0f, 4.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
